...
class SignetsAPIClientMock extends MockSignetsAPIClient {
  ...
  static void stubGetSessions(MockSignetsAPIClient mock, List<Session> sessionsToReturn) {
    when(mock.getSessions()).thenAnswer((_) async => sessionsToReturn);
  }

  /// Throw [exceptionToThrow] when [getSessions]
  static void stubGetSessionsException(MockSignetsAPIClient mock, {ApiException exceptionToThrow = signetsException}) {
    when(mock.getSessions()).thenThrow(exceptionToThrow);
  }

  /// Stub the answer of the [getPrograms]
  static void stubGetPrograms(MockSignetsAPIClient mock, List<Program> programsToReturn) {
    when(mock.getPrograms()).thenAnswer((_) async => programsToReturn);
  }

  /// Throw [exceptionToThrow] when [getPrograms]
  static void stubGetProgramsException(MockSignetsAPIClient mock, {ApiException exceptionToThrow = signetsException}) {
    when(mock.getPrograms()).thenThrow(exceptionToThrow);
  }

  /// Stub the answer of the [getInfo]
  static void stubGetInfo(MockSignetsAPIClient mock, ProfileStudent infoToReturn) {
    when(mock.getStudentInfo()).thenAnswer((_) async => infoToReturn);
  }

  /// Throw [exceptionToThrow] when [getInfo]
  static void stubGetInfoException(MockSignetsAPIClient mock, {ApiException exceptionToThrow = signetsException}) {
    when(mock.getStudentInfo()).thenThrow(exceptionToThrow);
  }

  /// Stub the answer of the [getCourses]
  static void stubGetCourses(MockSignetsAPIClient mock, {List<Course> coursesToReturn = const []}) {
