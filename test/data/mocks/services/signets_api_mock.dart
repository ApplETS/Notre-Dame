...
class SignetsAPIClientMock extends MockSignetsAPIClient {
  ...
  static void stubAuthenticate(MockSignetsAPIClient mock, {bool connected = false}) {
    // ignore: deprecated_member_use_from_same_package
    when(mock.authenticate(username: anyNamed("username"), password: anyNamed("password")))
  ...
  static void stubGetCoursesActivities(
    ...
    when(mock.getCoursesActivities(username: anyNamed("username"), password: anyNamed("password"), session: session))
  ...
  static void stubGetScheduleActivities(
    ...
    when(mock.getScheduleActivities(username: anyNamed("username"), password: anyNamed("password"), session: session))
        .thenAnswer((_) async => coursesActivitiesToReturn);
  }
  ...
  static void stubGetSessions(MockSignetsAPIClient mock, String username, List<Session> sessionsToReturn) {
  ...
  static void stubGetSessionsException(MockSignetsAPIClient mock, String username,
      {ApiException exceptionToThrow = signetsException}) {
    when(mock.getSessions(username: username, password: anyNamed("password"))).thenThrow(exceptionToThrow);
  }

  /// Stub the answer of the [getPrograms] when the [username] is used.
  static void stubGetPrograms(MockSignetsAPIClient mock, String username, List<Program> programsToReturn) {
  ...
  static void stubGetProgramsException(MockSignetsAPIClient mock, String username,
      {ApiException exceptionToThrow = signetsException}) {
    when(mock.getPrograms(username: username, password: anyNamed("password"))).thenThrow(exceptionToThrow);
  }

  /// Stub the answer of the [getInfo] when the [username] is used.
  static void stubGetInfo(MockSignetsAPIClient mock, String username, ProfileStudent infoToReturn) {
    when(mock.getStudentInfo(username: username, password: anyNamed("password"))).thenAnswer((_) async => infoToReturn);
  }
  ...
  static void stubGetInfoException(MockSignetsAPIClient mock, String username,
      {ApiException exceptionToThrow = signetsException}) {
    when(mock.getStudentInfo(username: username, password: anyNamed("password"))).thenThrow(exceptionToThrow);
  }

  /// Stub the answer of the [getCourses] when the [username] is used.
  static void stubGetCourses(MockSignetsAPIClient mock, String username, {List<Course> coursesToReturn = const []}) {
    when(mock.getCourses(username: username, password: anyNamed("password"))).thenAnswer((_) async => coursesToReturn);
  }

  /// Throw [exceptionToThrow] when [getCourses] with the [username] is used.
  static void stubGetCoursesException(MockSignetsAPIClient mock, String username,
      {ApiException exceptionToThrow = signetsException}) {
    when(mock.getCourses(username: username, password: anyNamed("password"))).thenThrow(exceptionToThrow);
  }

  /// Stub the answer of the [getCourseSummary] when the [username] and [course] is used.
  static void stubGetCourseSummary(MockSignetsAPIClient mock, String username, Course course,
      {CourseSummary? summaryToReturn}) {
    when(mock.getCourseSummary(username: username, course: course, password: anyNamed("password")))
        .thenAnswer((_) async => summaryToReturn!);
  }

  /// Throw [exceptionToThrow] when [getCourseSummary] with the [username] and [course] is used.
  static void stubGetCourseSummaryException(MockSignetsAPIClient mock, String username, Course course,
      {ApiException exceptionToThrow = signetsException}) {
    when(mock.getCourseSummary(username: username, course: course, password: anyNamed("password")))
        .thenThrow(exceptionToThrow);
  }
  ...
  static void stubGetCourseReviews(MockSignetsAPIClient mock, String username,
    ...
    when(mock.getCourseReviews(
            username: username, session: session ?? anyNamed('session'), password: anyNamed("password")))
        .thenAnswer((_) async => reviewsToReturn);
  }

  /// Throw [exceptionToThrow] when [getCoursesReviews] with the [username] and [session] is used.
  /// If [session] is null any session will be accepted.
  static void stubGetCourseReviewsException(MockSignetsAPIClient mock, String username,
