...
class NewsRepositoryMock extends MockNewsRepository {
  ...
  static void stubGetNews(NewsRepositoryMock mock, {PaginatedNews? toReturn, int pageNumber = 1, int pageSize = 3}) {
    when(mock.getNews(pageNumber: pageNumber, pageSize: pageSize)).thenAnswer((_) async => toReturn);
  }

  /// Stub the function [getNews] of [mock] when called will return [toReturn].
  static void stubGetNewsOrganizer(
    NewsRepositoryMock mock,
    String organizerId, {
    PaginatedNews? toReturn,
    int pageNumber = 1,
    int pageSize = 3,
  }) {
    when(
      mock.getNews(pageNumber: pageNumber, pageSize: pageSize, organizerId: organizerId),
    ).thenAnswer((_) async => toReturn);
  }

  /// Stub the function [getNews] of [mock] when called will throw [toThrow].
  static void stubGetNewsException(
    NewsRepositoryMock mock, {
    ...
    when(
      mock.getNews(pageNumber: pageNumber, pageSize: pageSize),
