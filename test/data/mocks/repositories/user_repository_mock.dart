...
class UserRepositoryMock extends MockUserRepository {
  ...
  static void stubGetInfoException(
    UserRepositoryMock mock, {
    Exception toThrow = const ApiException(prefix: 'ApiException'),
    bool? fromCacheOnly,
  }) {
    when(
      mock.getInfo(fromCacheOnly: fromCacheOnly ?? anyNamed("fromCacheOnly")),
  ...
  static void stubGetProgramsException(
    ...
    when(
      mock.getPrograms(fromCacheOnly: fromCacheOnly ?? anyNamed("fromCacheOnly")),
    ).thenAnswer((_) => Future.delayed(const Duration(milliseconds: 50)).then((value) => throw toThrow));
  }
}
