...
class CourseRepository {
  ...
  Future<List<ScheduleActivity>> getDefaultScheduleActivities(
    ...
    try {
      ...
      _logger.e("$tag - getDefaultScheduleActivities: exception raised will trying to load activities from cache.");
    }
    ...
    try {
      ...
      // Do nothing, the caching will retry later and the error has been logged by the [CacheManager]
      _logger.e("$tag - getScheduleActivities: exception raised will trying to update the cache.");
    }
  ...
  Future<List<ScheduleActivity>> getScheduleActivities({bool fromCacheOnly = false}) async {
    ...
    if (_scheduleActivities == null) {
      ...
      try {
        ...
        _logger.e("$tag - getScheduleActivities: exception raised will trying to load activities from cache.");
      }
    }
  ...
  Future<List<Course>> getCourses({bool fromCacheOnly = false}) async {
    ...
    if (_courses == null) {
      ...
      try {
        ...
        _logger.e("$tag - getCourses: exception raised will trying to load courses from cache.");
      }
    }
    ...
    try {
      ...
      // Do nothing, the caching will retry later and the error has been logged by the [CacheManager]
      _logger.e("$tag - getCourses: exception raised will trying to update the cache.");
    }
  ...
  Future<Course> getCourseSummary(Course course) async {
    ...
    try {
      ...
      _logger.e("$tag - getCourseSummary: exception raised will trying to update the cache.");
    }
  ...
  CourseReview? _getReviewForCourse(Course course, Map<String, List<CourseReview>> reviews) {
    // Todo: changer pour firstWhereOrNull après update de Collection
    final review =
        reviews[course.session]?.where((element) => element.acronym == course.acronym && element.group == course.group);
